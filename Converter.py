...
__author__ = "Mugilan Ganesan, transferred to Pillow 11 by UserNumber0x01"
__email__ = "deleted"
__status__ = "deleted"
__version__ = "1.2.0"
...
def translation(lines, font_data):
    ...
    for i in range(0, len(lines)):
        ...
        if y[-1] not in layer_height:
            layer_height[y[-1]] = draw.textbbox((0, 0), "a", font=font)[3] + font_size
        ...
        if line == "START" or line == "STOP":
            ...
            width = bbox[2] - bbox[0] + font_size * 2
            height = bbox[3] - bbox[1] + 2 * font_size
            ...
            bbox = draw.textbbox((0, 0), line, font=font)
            width = bbox[2] - bbox[0] + font_size * 2
            height = bbox[3] - bbox[1] + font_size
            ...
            if len(line) > len("IF") + 1:
                bbox = draw.textbbox((0, 0), line, font=font)
                width = 3 / 2 * (bbox[2] - bbox[0])
                height = 5 * (bbox[3] - bbox[1])
            else:
                width = 5 * font_size
                height = 5 * font_size
...
def drawer(chart_code, max_branch, max_y, layer_height, branch_width, font_data):
    ...
    def Terminator(text, position):
        ...
        width = bbox[2] - bbox[0] + font_size * 2
        height = bbox[3] - bbox[1] + font_size * 2
